...
checkpoint_spconv2_native = True
...
model = dict(
    ...
    criterion=dict(
        ...
        inst_criterion=dict(
            ...
            iter_matcher=True,
            # ACPE is the only added formal supervision.  The closed SAHQS
            # anchor loss is deliberately absent.
            fix_mean_loss=True)),
    train_cfg=dict(),
    test_cfg=dict(
        topk_insts=300,
...
load_from = ('<path to the published ForestFormer3D checkpoint>/'
             'ff3d_cqcf_v2_distinct_windows_20260730_r1/work_dirs/'
